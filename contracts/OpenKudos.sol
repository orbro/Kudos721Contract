...
contract OpenKudos is ERC721Token("OpenKudosToken", "OKDO"), Ownable {
    using SafeMath for uint256;

    public uint kudoIncrement = 0;

    struct Kudo {
        bytes32 kudoType; //New
        uint256 priceFinney;
...
//New
    struct KudoType {
        address[] owners;
        bytes32 typeMetadata; //Should be IPFS hash; store data in IPFS?
    }

//New
    mapping (uint => bytes32) KudosTypes;
...
//New
    function createKudosType(address[] _typeOwners, bytes32 _typeMetadata) {
        kudoIncrement ++;
        KudosTypes[kudoIncrement].owners = _typeOwners;
        KudosTypes[kudoIncrement].typeMetadata = _typeMetadata;
    }
    ...
    function mint(uint _kudoType, address _to, uint256 _priceFinney, uint256 _numClonesAllowed, string _tokenURI) public mintable onlyOwner returns (uint256 tokenId) {
